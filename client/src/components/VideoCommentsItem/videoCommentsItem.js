...
import VideoCommentsReplyItem from '../videoCommentsReplyItem/videoCommentsReplyItem'
import Cookies from 'js-cookie'
import axios from 'axios'
import '../articleCommentsItem/articleCommentsItem.scss'
...
class VideoCommentsItem extends Component {
	...
	render() {
		...
		if (this.state.replys && this.state.replys.length) {
			replys =
				this.state.replys.length > 3 && !this.state.isShowAllReply ? this.state.replys.slice(0, 3) : this.state.replys
			showMoreBtn =
				this.state.replys.length > 3 && !this.state.isShowAllReply && !this.state.isReplyFold ? (
		...
		return (
			<div className='forum-comment-item'>
				...
				<div className='forum-comment-user-details'>
					<a className='forum-comment-user-name'>{authorName}</a>
					<p className='forum-comment-user-content'>{commentContent}</p>
					<div className='forum-comment-operation'>
						<div className='forum-comment-operation-left'>
							{agreeAndAgainst.indexOf(_id) === -1 && !this.state.againstClick ? this.state.agreeClick ? (
							...
							{agreeAndAgainst.indexOf(_id) === -1 && !this.state.agreeClick ? this.state.againstClick ? (
								<a className='forum-comment-operation-agree'>
							...
							<a className='forum-comment-reply' onClick={() => this.foldReply()}>
								{this.state.isReplyFold ? `${this.replyLength()}个回复` : '收起回复'}
							</a>
							...
							<div className='my-reply-container'>
								...
								<div className='my-reply-container-right'>
									...
									<div className='my-reply-container-btn-wrapper'>
										<button className='my-reply-container-btn' onClick={() => this.reply()}>
											回复
										</button>
									</div>
								</div>
